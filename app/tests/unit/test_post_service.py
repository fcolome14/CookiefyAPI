...
def test_create_list_duplicate(mock_post_service):
    service, repo = mock_post_service
    repo.get_list_by_name.return_value = ListModel(id=1, name="List", owner=1)

    result = service.create_list(1, ListCreate(name="List", description="", is_public=True, accepts_contributions=False))
